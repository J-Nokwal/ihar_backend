...
func init() {
	...
	db = configs.GetDB()
	db.LogMode(true)
	db.AutoMigrate(&User{}, &Post{}, &Comment{}, &Likes{}, &Report{})
	...
	db.Model(&Report{}).AddForeignKey("post_id", "posts(id)", "CASCADE", "CASCADE")
	db.Model(&Report{}).AddForeignKey("user_id", "users(id)", "CASCADE", "CASCADE")
	db.Model(&Report{}).AddForeignKey("from_user_id", "users(id)", "CASCADE", "CASCADE")
